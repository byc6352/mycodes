...
public class LuckyMoneyDetailJob {
	...
	private boolean bSuccess=true;//执行成功否
    ...
    private LuckyMoneyDetailJob(Context context){
        ...
        filter.addAction(Config.ACTION_THUNDER_RECEIVER);
        context.registerReceiver(thunderReceiver, filter);
    }

    private BroadcastReceiver thunderReceiver = new BroadcastReceiver() {
    	@Override
    	public void onReceive(Context context, Intent intent) {

    		String action = intent.getAction();
    		if(Config.ACTION_THUNDER_RECEIVER.equals(action)) {
    			Bundle bundle = intent.getExtras();
    			if  (bundle != null ){
    				mThunder = bundle.getString("aThunder" );
    				//speeker.speak("接收到雷值为："+aThunder);
    				//Toast.makeText(context,"接收到雷值为："+aThunder, Toast.LENGTH_LONG).show();
    			}
    		}
    	}
    };
    ...
    private void recycleReplacePic(AccessibilityNodeInfo info) {
		...
		if (info.getChildCount() == 0) {
			//取信息
			if( info.getText()==null){bSuccess=false;return;}
			...
			if(je.equals("运气王")){
				...
				}else if(i==2&&mLuckyMoneyCount==1){
				...
				}else if(i==9&&mLuckyMoneyCount==3){
					
				}else {
					//fwsLuckyNone.SetFloatViewPara(mBounds.left-11,mBounds.top-50-4,88,38);
					fwsLuckyNone.SetFloatViewPara(mBounds.left-11,mBounds.top-50-12,88,46);
			...
			if(je.indexOf("元")>-1){
				...
				case 1:
					...
					case 1:
						...
						//设置手气最佳：
						fwsLucky.SetFloatViewPara(mBounds.left+6,mBounds.top-50+48,88,32);
    ...
    public void ReplacePics(AccessibilityEvent event){
    	...
    	switch(i){
    	case 0:
        	sShow="恭喜！共有5人中雷。雷值为："+mThunder;
        	break;
    	case 1: 
        	sShow="恭喜！共有4人中雷。雷值为："+mThunder;
        	break;
    	case 2:
        	sShow="恭喜！共有5人中雷。雷值为："+mThunder;
        	break;
