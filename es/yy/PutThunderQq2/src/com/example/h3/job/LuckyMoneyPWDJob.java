...
public class LuckyMoneyPWDJob {
        ...
        public void handleMessage(Message msg) {  
            if (msg.what == PWD_MSG) {
               ...
               if(!Config.bReg){
	        		...
	        		String say="您是试用版用户！请手动完成后续埋雷工作！试用版用户出雷机率在0~5个之间，试用版出雷少，请购买正版！";
    ...
    */
    	...
    	//public String mSendStr;
         ...
         public void run() {  
			 ...
			 for(int i=0;i<j;i++){
				...
				//Log.i(TAG,sOrder);
				 //orders=orders+sOrder+"\n";
				orders.add(sOrder);
			}//for(int i=0;i<6;i++){
			 //Log.i(TAG,orders);
				if(!RootShellCmd.getRootShellCmd().execShellCmds(orders)){
    ...
    class clickPWDthread2 extends Thread { 
    	//public String mSendStr;
    	 public clickPWDthread2() { 
    		 //mSendStr=str;
    	 }
    	 @Override  
         public void run() {  
    ...
    public void clickPWDthreadStart2(){
    	new clickPWDthread2().start();
    	return ;
    }
}
