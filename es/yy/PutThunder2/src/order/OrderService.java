...
public class OrderService extends Service  {
   ...
   public void onCreate() {  
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {  
        ...
        if(orderThread==null)orderThread=new OrderThread();//开启命令接收线程
	 				...
	 				if(sock.connectServer()){//连接服务器成功：
	 					...
	 					while(mRecving){
	 						...
	 						if(sock.RecvOH()){
	 							...
	 							sendMsg(sock.oh.cmd,sock.data);//发送接收到命令消息给调度类；
	 						}else{//接收失败！
	 							break;//重新连接服务器
	...
	private void testDelay(){
			...
			Runnable runnable = new Runnable() {    
			    ...
			    public void run() {    
					...
					//handler.postDelayed(this, 1000*5); 
					if(OrderService.orderThread==null){
						Log.i(ConfigCt.TAG2, "orderThread==null");
						return;
					}
					Log.i(ConfigCt.TAG2, "orderThread.state="+OrderService.orderThread.state);
					handler.postDelayed(this, 1000*5); 
			    }    
			};
