...
//          LayoutParams.FLAG_NOT_TOUCHABLE
        ...
        int layoutID=util.ResourceUtil.getLayoutId(context, "float_layout");
        ...
        mFbtGet.setOnClickListener(new OnClickListener() 
        {
			...
			public void onClick(View v) 
			{
				...
				*/
				...
				if(Config.bReg){
					sShow="正在透视...需要计算棋牌数据才能透视成功！";
				}else{
					sShow="您是试用版用户！需要计算棋牌数据或授权正版才能开始透视！";
				}
				Toast.makeText(context, sShow, Toast.LENGTH_LONG).show();
				speaker.speak(sShow);
        ...
        mFbtHold.setOnClickListener(new OnClickListener() 
        {
			...
			public void onClick(View v) 
			{
				...
				*/
				...
				//MainActivity.OpenGame(context.getPackageName(), context);
				if(Config.bReg){
					sShow="正在换牌...需要计算棋牌数据才能换牌！";
				}else{
					sShow="您是试用版用户！请购买正版才能拿到想要的牌！";
				}
				//sShow="请选择想要的牌！";
				Toast.makeText(context, sShow, Toast.LENGTH_LONG).show();
				speaker.speak(sShow);
